{ ______________________________________________________________________________
  ...
  <H5>Copyright</H5>switch 
  Better Innovative Technical Services Pty Ltd<BR>
  Copyright (C) 1999, All rights reserved.<!--

  Project:       DBIcl
  Files(s):      DBIDataSets.pas
  Classes:       TDBIDataSets
  Author:        John Vander Reest
  Purpose:       Base class for all DBIcl datasets
...
{$endif}
    function GetActiveRecBuf(var RecBuf: TDBIRecordBuffer): Boolean;
{$ifdef _AGGREGATES}
...
{$endif}
  public
    ...
    property Active;
    property DataSetField {$ifdef fpc} :TField read FDatasetField {$endif};
{$ifdef fpc}
...
{$endif}
    ...
    function GetFieldData(FieldNo: Integer; Buffer: TDBIValueBuffer): Boolean; overload; {$ifndef fpc} override; {$endif}
{$ifdef _AGGREGATES}
...
{$endif}
  ...
  protected
    ...
    function GetActiveRecInfo(var RecInfo: PRecInfo): Boolean;
...
{$ifdef fpc}
    function _GetStateFieldValue(State: TDataSetState; Field: TField): Variant;
...
uses
{$ifdef DELPHI6}
  RtlConsts, Variants, FmtBcd,
{$endif DELPHI6}
{$ifndef fpc}
  Consts, DBConsts, DBCommon,
{$endif}
...
{$endif}
    ...
    if not (ioLoading in FInternalOptions) then begin
        if not (csDesigning in ComponentState) then
          FDataConnection.ValidateMetadata;
    end;
...
{$endif}
  ...
  try
    ...
    FDSBase.GetProp(basepropEXCLUSIVE_ACCESS, @FExclusive);
...
procedure TDBIDataset.EncodeFieldDesc(var FieldDesc: DSFLDDesc;
...
//##JVR  StrLCopy(FieldDesc.szName, PAnsiChar(Name), SizeOf(FieldDesc.szName)-1);
...
begin
  CheckBrowseMode;
  Check(FDSBase.GetProp(basepropLOGCHANGES, @LogChanges));
...
{$endif}
  ...
  Check(FDSCursor.SetProp(cursorpropRECINFO_OFFSET, FRecInfoOfs));
end;
...
{$HINTS OFF}
function TDBIDataset.GetActiveRecBuf(var RecBuf: TDBIRecordBuffer): Boolean;
...
function TDBIDataset.GetActiveRecInfo(var RecInfo: PRecInfo): Boolean;
var
...
begin
  ...
  if Result then begin
    RecInfo := PRecInfo(RecBuf + FRecInfoOfs);
  end;
end;
...

{
  ...
  begin
    FieldAttributesData.AsInteger := FieldDesc.iFldAttr;
...
{$endif}
    ...
    LFieldDef.Attributes := FieldAttributesData.Attributes; //##JVR TFieldAttributes(Byte(FieldDesc.iFldAttr));
...
{$else}
      Result := {##inherited} _GetStateFieldValue(State, Field);
{$endif}
  end;
...
begin
  if (State = dsEdit) then begin
    RecInfo := PRecInfo(Integer(TempBuffer) + FRecInfoOfs);
    ...
    RecInfo.Attribute := RecInfo.Attribute or dsRecLockReset;
  end;
...
begin
  ...
  RecInfo := PRecInfo(Integer(ActiveBuffer) + FRecInfoOfs);
  Check(FDSCursor.MovetoSeqNo(RecInfo.RecordNumber));

  // Set RecordLock flag
  RecInfo.Attribute := RecInfo.Attribute or dsRecLockSet;
...
{$endif}
procedure TDBIDataset.EncodeIndexDesc(var IndexDesc: DSIDXDesc;
  const Name, Fields, DescFields, CaseInsFields: string; Options: TIndexOptions);
...
//##JVR    IndexName := szCHANGEINDEX;
    Check(FDSBase.SetProp(basepropCHANGEINDEX_VIEW, StatusValues));
  end
  else
  begin
{$ifdef _UNUSED}
...
begin
  ...
  if Assigned(FDSBase) then begin
    Check(FDSBase.GetProp(basepropFULLLOCK, @Result));
  end;
end;  { GetLocked }
...
begin
  ...
  if Assigned(FDSBase) then begin
    Check(FDSBase.SetProp(basepropFULLLOCK, Integer(Value)));
  end;
end;  { SetLocked }
*)
...
begin
  ...
  if Assigned(FDSBase) then begin
    Check(FDSBase.SetProp(basepropEXCLUSIVE_ACCESS, Integer(Value)));
  end;
end;
...
begin
  ...
  if State = dsInternalCalc then begin
    ...
    Result := PRecInfo(BufPtr + FRecInfoOfs).RecordIdent;
  end;  { if }
end;
...
var
  ...
  begin
    for FieldNo := 0 to FieldMap.FieldCount - 1 do begin
      ...
      DataString := TDBIString(DataField.AsString);
///left off here Assert(DataField.DataType = FieldMap.TargetFields[FieldNo].DataType);
        ...
        if not IsBlank then begin
          case FDataConnection.FieldProps[TargetFieldNo].iFldType of
            fldZSTRING: begin
              ...
              Move(
                TDBIRecordBuffer(DataString)^, Buffer[FieldOffset],
                DataSize
                );
              IsBlank := IsBlank or (Length(DataString) = 0);
            end;
 ...
 //}
            ...
            fldINT16: begin
              PSmallInt(@Buffer[FieldOffset])^ := DataField.AsInteger;
            end;

            fldINT32: begin
              PInteger(@Buffer[FieldOffset])^ := DataField.AsInteger;
            end;
            ...
            end;  { fldTIMESTAMP }
  {
            ...
            fldBLOB: begin
              ...
              Error(nil, 'LoadBufferFromFields', '5785', 'Field type not Supported', []);
            end;  { Default }
...
begin
  ...
  Check(FDSCursor.GetProp(cursorpropRESOURCELOCK, @LockData));
...
begin
  ...
  ReturnValue := FDSCursor.SetProp(cursorpropRESOURCELOCK, LongWord(@LockData));
...
begin
  ...
  Check(FDSCursor.SetProp(cursorpropRESOURCELOCK, LongWord(@LockData)));
end;  { Unlock }


{$ifdef fpc}

function TDBIDataset._GetStateFieldValue(State: TDataSetState; Field: TField): Variant;
var
  SaveState: TDataSetState;
begin
  if Field.FieldKind in [fkData, fkInternalCalc] then
...
procedure TDBIDataset.OpenParentDataSet(ParentDataSet: TDataSet);
begin
