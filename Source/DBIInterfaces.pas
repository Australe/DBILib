...
{**
  ...
  Project:       DBILib
  Files(s):      DBIInterfaces.pas
  Classes:       TDBIbase, TDBICursor, TDBIDataConnection
  Author:        John Vander Reest
  Purpose:       Standalone Dataset Interfaces
...
uses
  Classes, SysUtils, TypInfo,  DBIIntfConsts, DB, Contnrs, DBIConst, DBIStrings, DBIFilters, DBIIndices;

type
  PDBIPointerList = ^TDBIPointerList;
  TDBIPointerList = array[0..$FFFE] of Pointer;
...
{$endif}
  ...
  TDBIFieldAttributesData = record
    case Boolean of
      True: (Attributes: TFieldAttributes);
...
const
  FieldUnits1Map: array[0..MAXLOGFLDTYPES-1] of Word = (
    0,                                 // fldUNKNOWN
    0,                                 // fldZSTRING
    1,                                 // fldDATE
    1,                                 // fldBLOB
...
{$endif}
    ...
    function GetProp(                   { Get property }
        eProp       : TDBIXBaseProperty; //##JVR DSProp;
        piPropValue : Pointer
    ): DBIResult;
...
{$endif}
    ...
    function GetProp(               { Get property }
        eProp        : TDBICursorProperty; //##JVR CURProp;
        piPropValue  : Pointer
    ): DBIResult;
...
{$endif}
    ...
    function SetProp(               { Set property }
        eProp       : TDBICursorProperty; //##JVR CURProp;      { Property to set }
...
type
  IDBIAppServer = interface //##JVR(IDispatch)
//##JVR    ['{1AEFCC20-7A24-11D2-98B0-C69BEB4B5B6D}']
...
//##JVR    procedure SetRecordCount(const Value: Integer); virtual; abstract;
//##JVR    property RecordCount: Integer index DSAttr(dsRecAll) read GetCount;
...
//##JVR    procedure Sync(const Position: Integer; const Buffer); virtual; abstract;
...
//##JVR    FDSFldProps: TFieldDescList;  // Array of Field description properties
...
//##JVR    function AppendData(Packet: TDBIDataArray; bEof: LongBool): DBIResult;
    function GetProps(var Prop: DSProps): DBIResult;
    ...
    function GetProp(eProp: TDBIXBaseProperty; piPropValue: Pointer): DBIResult;
    function SetProp(eProp: TDBIXBaseProperty; iPropValue: TDBIPropValue): DBIResult;
  ...
  protected
    ...
    function GetCursorProps(var p1: DSProps): DBIResult; virtual;
{##CONNECTION
    function GetDataConnection: TDBIDataConnection;
//}
    function GetIndexDescs(bCurrentOnly: LongBool;
      var IdxDesc: DSIDXDesc): DBIResult; virtual;
    ...
    function GetSequenceNumber(var iSeq: LongWord): DBIResult; virtual;
    function GetRecordAttribute(var Attr: DSAttr): DBIResult; virtual;
    function GetRecordCount(var iRecs: Integer): DBIResult; virtual;
    ...
    function MoveToBookMark(pBookMark: Pointer): DBIResult; virtual;
{
    ...
    function GetProp(eProp: TDBICursorProperty; piPropValue: Pointer): DBIResult; virtual;
    function SetProp(eProp: TDBICursorProperty; iPropValue: TDBIPropValue): DBIResult; virtual;
    function GetRecordNumber(var iRecNo: LongWord): DBIResult; virtual;
...
begin
  try
    ...
    with FFieldProps[FieldNo] do begin
      iFieldID := FieldNo + 1;
      iFldLen := GetFieldSize(iFldType, iFldSubType, iUnits1, iUnits2);
...
{$endif}
  ...
  var
    FieldAttributesData: TDBIFieldAttributesData;
  begin
    FieldAttributesData.Attributes := Attributes;
    FillChar(FieldDesc, SizeOf(FieldDesc), #0);
    StrLCopy(FieldDesc.szName, TDBINameBuffer(Name), SizeOf(FieldDesc.szName)-1);

    FieldDesc.iFldType := FieldTypeMap[DataType];
    FieldDesc.iUnits1 := FieldUnits1Map[FieldDesc.iFldType];
    if FieldDesc.iFldType = fldWIDESTRING  then begin
      FieldDesc.iFldType := fldUNICODE;
    end;

    FieldDesc.iFldSubType := FldSubTypeMap[DataType];
    FieldDesc.bCalculated := Calculated;
    FieldDesc.iFldAttr := FieldAttributesData.AsInteger;
    ...
    case DataType of
      ...
      ftBCD {$ifdef DELPHI6}, ftFMTBcd {$endif} : begin
        { Default precision is 32, Size = Scale }
        if (Precision > 0) and (Precision <= 32) then begin
  ...
  begin
    ...
    for FieldNo := 0 to FieldDefs.Count - 1 do begin
      with FieldDefs[FieldNo] do begin
...
begin
  //##JVR - elevated from the derived classes
...
{##JVR
    Move(
      (@FFieldProps[0])^,
      Fields^,
      SizeOf(FFieldProps[0]) * FieldCount
      );
}      
  end;
end;  { GetFieldProps }
...
begin
  try
    ...
    Error(nil, Caller, '985', 'Failed to set dataset field properties', []);
  end;  { try..except }
end;  { SetFieldProps }
...
procedure TDBIDataConnection.Error(
  E: Exception;
  const Caller: String;
  const Reference: String;
  const ErrMsg: String;
...
// =============================================================================
// TDBIDataConnection protected methods
// =============================================================================
...
function TDBIDataConnection.GetFieldSize(
  var DataType: Integer;
  var SubType: Integer;
  var Size: Integer;
  var Decimals: Integer
...
//  fldBLOB:       See below!
    ...
    fldBYTES:      Result := Size;
    fldTIME:       Result := sizeofTIME;
    fldTIMESTAMP:  Result := sizeofDATETIME;
    fldUINT16:     Result := sizeofINT16;
    fldUINT32:     Result := sizeofINT32;
    fldFLOATIEEE:  Result := sizeofUNKNOWN;
    fldVARBYTES:   Result := sizeofUNKNOWN;
    fldLOCKINFO:   Result := sizeofUNKNOWN;
    fldCURSOR:     Result := sizeofUNKNOWN;
    fldINT64:      Result := sizeofINT64;
    fldUINT64:     Result := sizeofINT64;
    fldADT:        Result := sizeofADT;
    fldARRAY:      Result := sizeofUNKNOWN;
    fldREF:        Result := sizeofUNKNOWN;
    fldTABLE:      Result := sizeofTABLE;
    fldDATETIME:   Result := sizeofDATETIME;
    fldFMTBCD:     Result := SizeOf(TBcd);
    fldWIDESTRING: Result := Size + 2;
    fldUNICODE:    Result := Size + 2;
  ...
  if (Result <= 0) then begin
    Error(nil, 'GetFieldSize', '1600', 'Unknown Data Type "%d"', [DataType]);
  end;
end;  { GetFieldSize }
...
// =============================================================================
// TDBIDataConnection protected methods to access FieldProps data as dataset
// =============================================================================
...
begin
  ...
  Stream := DBIFileStreams.CreateFileStream(AFileName, fmOpenRead);
  try
...
{##JVR
  ...
  Stream := DBIFileStreams.CreateFileStream(AFileName, fmCreate);
  try
...
begin
  ...
  if {##JVR, this is a mistake I believe -->not }Assigned(DataInfo) then begin
...
{##JVR
  // If the found Lock item has a Mode, smaller or equal to the passed in value
  if (LockItem.FLockData.iMode <= ALockData.iMode) then begin
  end;
}
end;  { Locked }
...
//##JVR-NOP      Result := DBIERR_NOTLOCKED;
    end;
  end
...
// =============================================================================
// TDBIBase Public Methods as Exposed in DBIIntf.pas
// =============================================================================
...
function TDBIBase.GetProps(var Prop: DSProps): DBIResult;
begin
  Result := DBIERR_NONE;
  Assert(Assigned(FDataConnection));
...
//##JVR  Assert(Length(TFieldDescList(Fields)) > 0);
end;  { GetFieldDescs }
...
//##JVR  FDSProps.iIndexes := FIndices.Count;
//##JVR  Im doing it this way in order to standardise
  SetProp(basePropINDEXCOUNT, FIndices.Count);
...
function TDBIBase.GetProp(eProp: TDBIXBaseProperty; piPropValue: Pointer): DBIResult;
const
  Caller = 'GetProp';
...
begin
  ...
  case eProp of
    basepropRESOURCELOCK: begin
      FLocks.Locked(PDBILockData(piPropValue)^);
    end;  { basepropRESOURCELOCK }

    basepropREADONLY: begin
      Boolean(piPropValue^) := FDataConnection.ReadOnly;
    end;  { basepropREADONLY }

    basepropEXCLUSIVE_ACCESS: begin
      Boolean(piPropValue^) := Boolean(FDSProps.iUnused[auxExclusiveAccess]);
    end;  { basepropEXCLUSIVE }

    basepropDATACONNECTION: begin
      TDBIDataConnection(piPropValue^) := FDataConnection;
    end;  { basepropDATACONNECTION }

    basepropINDICES: begin
      TDBIIndexList(piPropValue^) := FIndices;
    end;  { basepropINDICES }

    basepropINDEXCOUNT: begin
      Integer(piPropValue^) := FDSProps.iIndexes;
    end;  { basepropINDEXCOUNT }

    basepropLOCKLIST: begin
      TDBILockList(piPropValue^) := FLocks;
    end;  { basepropLOCKLIST }

    basepropCHANGEINDEX_VIEW: begin
      DSAttr(Byte(piPropValue^)) := DSAttr(Byte((FDSProps.iUnused[auxStatusFilter])));
    end;  { basepropCHANGEINDEX_VIEW }

    else begin
      Error(nil,
        'GetProp', '2600',
        'Getting property of type "%s" not supported yet!',
        [GetEnumName(TypeInfo(TDBIXBaseProperty), Ord(eProp))]
        );
    end;  { Default }
  end;  { case }
end;  { GetProp }
...
function TDBIBase.SetProp(eProp: TDBIXBaseProperty; iPropValue: TDBIPropValue): DBIResult;
const
  Caller = 'SetProp';
...
begin
  ...
  case eProp of
    basepropRESOURCELOCK: begin
      if PDBILockData(iPropValue).bLocked then begin
        Result := FLocks.Add(PDBILockData(iPropValue)^);
      end
      else begin
        Result := FLocks.Remove(PDBILockData(iPropValue)^);
      end;
    end;  { basepropRESOURCELOCK }
...
// =============================================================================
// TXBaseDBInterface Protected Methods
// =============================================================================
...
{$ifdef DebugExceptions}
  ...
  DebugInfo := UnitName + '::' + Self.ClassName + '::' + Caller + '::' + Reference + #13;
{$else}
  DebugInfo := '';
{$endif}
...
// =============================================================================
// TDBICursor Public Methods as Exposed in DBIIntf.pas
// =============================================================================
...
begin
  ...
  FDSBase.GetProp(basepropDATACONNECTION, @FDataConnection);
  FDSBase.GetProp(basepropINDICES, @FIndices);
  FDSBase.GetProp(basepropLOCKLIST, @FLocks);
...
begin
  ...
  Cursor.GetProp(cursorpropDSBASE, @FDSBase);
  FDSBase.GetProp(basepropDATACONNECTION, @FDataConnection);
  FDSBase.GetProp(basepropINDICES, @FIndices);
  FDSBase.GetProp(basepropLOCKLIST, @FLocks);

  Cursor.GetProp(cursorpropRECORDPOSITION, @FPosition);
  Cursor.GetProp(cursorpropSELECTED_INDEX, @FSelectedIndex);
  Cursor.GetProp(cursorpropRECINFO_OFFSET, @FRecInfoOffset);
  FPriorIndex := FSelectedIndex;
...
function TDBICursor.GetCursorProps(var p1: DSProps): DBIResult;
begin
...
(*##CONNECTION - I might want to reinstate this in the future to replace FDataConnection
// _____________________________________________________________________________
{**
...
begin
  ...
  FDSBase.GetProp(basepropDATACONNECTION, @Result);
...
begin
  ...
  try
    ...
    Check(FDSBase.GetProp(basepropCHANGEINDEX_VIEW, @StatusFilter));

    // Get the RecInfo
    RecInfo := Pointer(Integer(pRecBuf) + FRecInfoOffset);
    try
      // Just a check to make sure that it's ok to reuse dsRecOrg
      if (RecInfo.Attribute and dsRecLockReset) = dsRecOrg then begin
      ...
      if ((RecInfo.Attribute and dsRecLockReset) = dsRecLockSet) then begin
      ...
      RecInfo.Data := DataInfo.Data;

      // Unlock record if requested (edit mode)
      if ((RecInfo.Attribute and dsRecLockReset) = dsRecLockReset) then begin
...
//##JVR      if (RecordAttribute and StatusFilter) = RecordAttribute then begin
      ...
      RecInfo.Attribute := RecInfo.Attribute and dsRecLockClear;
    end;  { try..finally }
...
function TDBICursor.GetSequenceNumber(var iSeq: LongWord): DBIResult;
begin
  Result := DBIERR_NONE;
  iSeq := Position;
...
function TDBICursor.GetRecordAttribute(var Attr: DSAttr): DBIResult;
begin
  Result := DBIERR_NONE;
...
function TDBICursor.GetRecordCount(var iRecs: Integer): DBIResult;
begin
  Result := DBIERR_NONE;
...
//##JVR  RecordAttribute := dsRecUnmodified;
  ...
  try
    ...
    FDSBase.GetProp(basepropCHANGEINDEX_VIEW, @StatusFilter);
...
begin
  ...
  // Initialise the Recordbuffer before using it to prevent any unexpected results
  with PRecInfo(Integer(pRecBuf) + FRecInfoOffset)^ do begin
    BookMarkFlag := bfInserted;
    RecordNumber := -1;
    RecordIdent := -1;
    Attribute := dsRecUnmodified;
    Data := nil;
  end;
...
// _____________________________________________________________________________
{**
  TDBICursor.InsertRecord - 21/03/2000 14:03:52.<P>
}
...
{$endif}
  ...
  begin
    ...
    if Assigned(pValue) then begin
      Result := LongBool(PPointerList(pValue)^[Offset]);
    end;
  end;  { GetBoolValue }
...
begin
  ...
  for Index := 0 to iFields - 1 do begin
    FDSLocateProps.iKeyFields[Index] := Integer(PDBIPointerList(piFields)^[Index]);
...
{$ifdef _AGGREGATES}
  ...
  Index := Integer(PPointerList(piFields)^[0]);
...
function TDBICursor.GetRecordNumber(var iRecNo: LongWord): DBIResult;
begin
  Result := DBIERR_NONE;
  Assert(Assigned(FIndices));
...
//##JVR  if FIndices.IndexOf(IndexDesc.szName) >= 0 then begin
  ...
  // Tell DataSet how many Indices there are
  FDSBase.SetProp(basePropINDEXCOUNT, FIndices.Count);
  ...
  try
    ...
    while (LocalCursor.Position < RecCount) and (Status = DBIERR_NONE) do begin
      ...
      NewIndex.InsertItem(
        pRecBuf,
        @(FFieldProps[IndexDesc.iKeyFields[0]-1]),
...
begin
  ...
  if FFilterLock and not Lock then begin
    ...
    Result := FDSBase.SetProp(basepropRESOURCELOCK, LongWord(@LockData));
  end;
end;  { SetLock }
  ...
  begin
    ...
    if (Result <> DBIERR_NONE) then begin
      ...
      while (SeqNo > 0) do begin
        ...
        FDSBase.SetProp(basepropRESOURCELOCK, LongWord(@LockData));
...
function TDBICursor.GetProp(eProp: TDBICursorProperty; piPropValue: Pointer): DBIResult;
var
  ...
  case eProp of
    ...
    cursorpropRESOURCELOCK: begin
      ...
      if (PLockData.iType = ltFilterLock) then begin
        ...
        Result := FDSBase.GetProp(basePropRESOURCELOCK, PLockData);
      end;
    end;  { cursorpropRESOURCELOCK }

    cursorpropSELECTED_INDEX: begin
      Integer(piPropValue^) := FSelectedIndex;
    end;  { cursorpropSELECTED_INDEX }

    cursorpropRECINFO_OFFSET: begin
      Integer(piPropValue^) := FRecInfoOffset;
  ...
  case eProp of
    ...
    cursorpropRESOURCELOCK: begin
      ...
      if (PLockData.iType = ltFilterLock) then begin
        ...
        Result := FDSBase.SetProp(basePropRESOURCELOCK, LongWord(PLockData));
      end;
    end;  { cursorpropRESOURCELOCK }
